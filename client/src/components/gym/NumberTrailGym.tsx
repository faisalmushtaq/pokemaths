      ...
      <div className="flex-1 w-full overflow-y-auto" style={{ backgroundImage: "linear-gradient(rgba(4,20,23,0.46), rgba(7,12,30,0.88)), url('/images/viridian-number-trail-bg.webp')", backgroundPosition: 'center', backgroundSize: 'cover' }}>
        <div className="w-full flex flex-col items-center" style={{ minHeight: '100%', padding: 'clamp(1rem,4vw,2rem) 1rem' }}>
          <div className="w-full rounded-2xl" style={{ maxWidth: '32rem', padding: 'clamp(1.1rem,5vw,1.8rem)', background: 'rgba(5,20,31,0.9)', border: '2px solid #34d399', boxShadow: '0 0 28px rgba(52,211,153,0.28)', textAlign: 'center' }}>
            <div style={{ fontSize: 'clamp(2rem,10vw,3rem)', marginBottom: '0.6rem', color: '#86efac' }}><PixelIcon name="forest" size="0.9em" /></div>
            ...
            <div className="flex flex-col gap-2">
              <button type="button" onClick={onReturnToJourney} className="w-full rounded-xl" style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.5rem,2.4vw,0.7rem)', padding: '0.85rem 0.7rem', background: 'linear-gradient(135deg, #facc15, #f59e0b)', color: '#111827', border: '2px solid #fde68a', cursor: 'pointer' }}><PixelIconLabel name="arrowRight" size="0.82em">RETURN TO JOURNEY</PixelIconLabel></button>
              <button type="button" onClick={() => { setChallengeIndex(0); setSelected(TRAIL_CHALLENGES[0].start); setChecked('idle'); setShowHint(false); setComplete(false); }} className="w-full rounded-xl" style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.42rem,2vw,0.58rem)', padding: '0.7rem', background: 'rgba(15,23,42,0.82)', color: '#7dd3fc', border: '1px solid #38bdf8', cursor: 'pointer' }}><PixelIconLabel name="reset" size="0.82em">PRACTISE AGAIN</PixelIconLabel></button>
              <button type="button" onClick={onBack} className="w-full rounded-xl" style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.38rem,1.8vw,0.52rem)', padding: '0.65rem', background: 'transparent', color: '#cbd5e1', border: '1px solid #64748b', cursor: 'pointer' }}><PixelIconLabel name="arrowLeft" size="0.82em">{backLabel}</PixelIconLabel></button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full flex flex-col gap-3" style={{ maxWidth: '35rem' }}>
          ...
          <section className="rounded-2xl" style={{ padding: 'clamp(0.95rem,4vw,1.4rem)', background: 'rgba(7,16,30,0.9)', border: '2px solid #facc15', boxShadow: '0 8px 28px rgba(0,0,0,0.38)' }}>
            ...
            <div className="grid grid-cols-3" style={{ gap: '0.45rem', marginTop: '1rem' }}>
              <button type="button" onClick={resetTrail} style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.34rem,1.7vw,0.48rem)', padding: '0.68rem 0.35rem', background: 'rgba(148,163,184,0.13)', color: '#e2e8f0', border: '1px solid #64748b', borderRadius: '0.55rem', cursor: 'pointer' }}><PixelIconLabel name="reset" size="0.8em">RESET</PixelIconLabel></button>
              <button type="button" onClick={() => setShowHint((current) => !current)} style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.34rem,1.7vw,0.48rem)', padding: '0.68rem 0.35rem', background: 'rgba(56,189,248,0.12)', color: '#7dd3fc', border: '1px solid #38bdf8', borderRadius: '0.55rem', cursor: 'pointer' }}><PixelIconLabel name="hint" size="0.8em">HINT</PixelIconLabel></button>
              {checked === 'correct' ? (
                <button type="button" onClick={nextChallenge} style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.34rem,1.7vw,0.48rem)', padding: '0.68rem 0.35rem', background: 'linear-gradient(135deg, #facc15, #f59e0b)', color: '#111827', border: '1px solid #fde68a', borderRadius: '0.55rem', cursor: 'pointer' }}><PixelIconLabel name="arrowRight" size="0.8em">NEXT</PixelIconLabel></button>
              ) : (
                <button type="button" onClick={checkTrail} style={{ fontFamily: PIXEL_FONT, fontSize: 'clamp(0.34rem,1.7vw,0.48rem)', padding: '0.68rem 0.35rem', background: 'linear-gradient(135deg, #34d399, #059669)', color: '#ecfdf5', border: '1px solid #6ee7b7', borderRadius: '0.55rem', cursor: 'pointer' }}><PixelIconLabel name="check" size="0.8em">CHECK</PixelIconLabel></button>
              )}
            </div>
          </section>

          <button type="button" onClick={onBack} style={{ alignSelf: 'center', fontFamily: PIXEL_FONT, fontSize: 'clamp(0.36rem,1.8vw,0.5rem)', color: '#cbd5e1', background: 'rgba(15,23,42,0.74)', border: '1px solid #64748b', borderRadius: '0.5rem', padding: '0.65rem 0.9rem', cursor: 'pointer' }}><PixelIconLabel name="arrowLeft" size="0.8em">{backLabel}</PixelIconLabel></button>
        </div>
      </div>
    </div>
  );
